...
- **主体先于关键词**：`发给我 / 小窗我` 里的“我”按收件人处理；`我需要你……` 是需求，不记成说话人的承诺。
- **否定按信号类型收敛**：`没有报错` 不再生成风险，`还没有完成` 不再生成进展；同一条消息里的其他事实仍可继续被识别。
- **KR 用完整挖掘窗口对齐**：展示证据仍固定 60 字，但长消息后半段的关键词不会被漏掉；多个 KR 命中时报告 `歧义（…）`，把裁决权留给人。

- 第一人称门槛仍是保守策略：无主语口语承诺（"明天之前搞定"）会漏；复杂转折句的主体指向也可能需要人工复核。
